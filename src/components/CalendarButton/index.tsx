...
export const CalendarButton = ({number, dayName, borderColor, backgroundColor, ...rest}:CardButtonProps) => {
    return (
        <div className={`justify-items-center content-center w-[50px] flex flex-col items-center gap-[7px] h-[72px] overflow-hidden rounded-[10px] relative ${borderColor === "selected" ? "bg-secondary-500" : borderColor === "unselected" ? "bg-grey-0" : borderColor === "passedMonth" ? "bg-[#f6f6f6]" : borderColor === "streak" ? "bg-tertiary-300" : borderColor === "failedStreak" ? "bg-primary-300" : "" } ${borderColor === "streak" || borderColor === "failedStreak" ? "border border-solid" : "border-[0.01px] border-solid"} ${borderColor === "selected" ? "border-secondary-600" : borderColor === "unselected" ? "border-grey-300" : borderColor === "passedMonth" ? "border-grey-300" : borderColor === "streak" ? "border-tertiary-500" : borderColor === "failedStreak" ? "border-primary-500" : ""} ${borderColor === "unselected" ? "px-[15px] py-[18px]" : "pt-[17px] pb-[18px] px-[15px]"}`} >
            ...
            <div className={`justify-items-center content-center tracking-[0] text-[11px] relative h-[11px] [font-family:'SF_Pro-Regular',Helvetica] text-center whitespace-nowrap leading-[normal] ${borderColor === "unselected" ? "w-[25px] mr-[-2.50px] ml-[-2.50px]" : "w-[22px] mr-[-1.00px] ml-[-1.00px]"} ${ borderColor === "selected" ? "text-grey-0" : borderColor === "streak" ? "text-tertiary-500" : borderColor === "failedStreak" ? "text-primary-500" : "text-grey-600"}`}>
                {dayName}
            </div>
        </div>
    )
